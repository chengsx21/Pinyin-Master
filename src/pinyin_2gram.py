"""
这个模块基于二元汉字模型, 实现了 viterbi 算法.
"""

import json
from utils import main

ALPHA = 0.001                                 # 平衡因子, 用于处理二元词语出现为次数为 0 的情况
BETA = 1 - ALPHA                              # 平衡因子, 用于处理二元词语出现为次数为 0 的情况

with open('pinyin-dict/pinyin_dict.json', 'r', encoding='utf-8') as f:
    pinyin_dict = json.load(f)
with open('frequency-list/sina/sina_2gram.json', 'r', encoding='utf-8') as f:
